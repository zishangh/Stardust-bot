...
STARDUST_AFK_FILE = "stardust_afk_only.json"

def load_stardust_afk():
    if os.path.exists(STARDUST_AFK_FILE):
        with open(STARDUST_AFK_FILE, "r") as f:
            try: return json.load(f)
            except: return {}
    return {}

def save_stardust_afk(data):
    with open(STARDUST_AFK_FILE, "w") as f:
        json.dump(data, f, indent=4)

# ==========================================
# 💤 MODULE: PREMIUM AFK SYSTEM ONLY
# ==========================================

# 1. SLASH COMMAND TO SET AFK
...
async def afk_command(interaction: discord.Interaction, reason: str = "Inertia/Away"):
    user_id = str(interaction.user.id)
    afk_data = load_stardust_afk()
    
    afk_data[user_id] = reason
    save_stardust_afk(afk_data)
    
    embed = discord.Embed(
        title="💤 AFK System Activated",
        description=f"{interaction.user.mention} is now status-logged as **AFK**.\n\n📝 **Reason:** `{reason}`",
        color=0xF5EAE1 # Soft Almond website theme color
    )
    await interaction.response.send_message(embed=embed)

# 2. SEPARATE LISTENERS TO PREVENT ON_MESSAGE CLASH
@bot.listen('on_message')
async def check_afk_mentions(message: discord.Message):
    if message.author.bot or not message.guild:
        return
    ...
    # A. AFK REMOVER: User back aa gaya
    if author_id in afk_data:
        del afk_data[author_id]
        save_stardust_afk(afk_data)
        welcome_embed = discord.Embed(
            description=f"✨ Welcome back {message.author.mention}! Your **AFK profile** has been cleared.",
            color=0xF5EAE1
        )
        await message.channel.send(embed=welcome_embed, delete_after=5)

    # B. AFK RESPONDER: Kisi ne AFK user ko ping kiya
    if message.mentions:
